...
public class BoardController {
	...
	public ModelAndView view(@PathVariable("idx") int idx) {
		ModelAndView mav = new ModelAndView("/board/view");	// pathvariable을 쓰면 무조건 board부터 적어줘야한다.
	...
	@GetMapping("/write")
	public void write() {}	// void로 처리하는 건 넣어줄 내용이 없으니까 그냥 이렇게 해 ~
	...
	public ModelAndView modify(@PathVariable("idx") int idx) {
		ModelAndView mav = new ModelAndView("/board/modify"); // ()안에 아무것도 안 적혀 있으면 그냥  void와 같다고 생각하쟈
		BoardDTO dto = boardService.getBoard(idx);
		mav.addObject("dto", dto);
		return mav;
	}
	...
	public ModelAndView modify(BoardDTO dto) {
		...
		String url = "/board/view/" + dto.getIdx(); //"{idx}"라고 하면 안 돼 ㅠㅠ 이건 스크립트
		mav.addObject("msg", msg);
		mav.addObject("url", url);
		return mav;
	}
